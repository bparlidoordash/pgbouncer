...
check_variables(){
    test -n "$DB_HOST" ||
      test -n "$DB" ||
        error "You have to set the DB_HOST or DB Environment variable at the very least"
}
...
create_databases_config(){
  if [ -n "${DB}" ] ; then
    ...
    cat << EOF
${DB_CLIENT_SIDE_DBNAME:-*} = host = ${DB_HOST} \
port=${DB_PORT:-5432}\
${DB_USER:+" user=${DB_USER}"}\
${DB_PASSWORD:+" password=${DB_PASSWORD}"}\
${DB_DBNAME:+" dbname=${DB_DBNAME}"}\
${DB_AUTH_USER:+" auth_user=${DB_AUTH_USER}"}\
${DB_POOL_SIZE:+" pool_size=${DB_POOL_SIZE}"}\
${DB_RESERVE_POOL:+" reserve_pool=${DB_RESERVE_POOL}"}\
${DB_CONNECT_QUERY:+" connect_query=${DB_CONNECT_QUERY}"}\
${DB_POOL_MODE:+" pool_mode=${DB_POOL_MODE}"}\
${DB_MAX_DB_CONNECTIONS:+" max_db_connections=${DB_MAX_DB_CONNECTIONS}"}\
${DB_CLIENT_ENCODING:+" client_encoding=${DB_CLIENT_ENCODING}"}\
${DB_DATESTYLE:+" datestyle=${DB_DATESTYLE}"}\
${DB_TIMEZONE:+" timezone=${DB_TIMEZONE}"}
EOF
  fi
}
